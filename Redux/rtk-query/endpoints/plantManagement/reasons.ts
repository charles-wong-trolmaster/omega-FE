...
import { Reason } from "@/types/cultivation/ReasonType";

const reasonsApi = cultivationClient.injectEndpoints({
  endpoints: (builder) => ({
    getFullListOfReasons: builder.query({
      query: () => `/reasons`,
      providesTags: () => [{ type: "Reasons", id: "FULL_LIST" }],
      transformResponse: (response: { result: Reason[] }) => response.result,
    }),
    getListOfReasons: builder.infiniteQuery({
      query: (args) => {
        const {
      ...
      providesTags: () => [{ type: "Reasons", id: "LIST" }],
...
export const {
  useGetFullListOfReasonsQuery,
  useGetListOfReasonsInfiniteQuery,
} = reasonsApi;
